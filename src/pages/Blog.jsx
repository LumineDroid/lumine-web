...
const parseChangelog = content => {
    ...
    lines.forEach(line => {
        ...
        // Check for date line (format: YYYY-MM-DD)
        if (trimmedLine.match(/^\d{4}-\d{2}-\d{2}$/)) {
            ...
            // Extract version (starts with v)
            if (trimmedLine.match(/^v\d+\.\d+/) && !currentEntry.version) {
                currentEntry.version = trimmedLine;
            }
            // Extract changes (lines starting with -)
            else if (trimmedLine.match(/^-\s+/) && trimmedLine.length > 0) {
                const change = trimmedLine.replace(/^-\s+/, "").trim();
                if (change) {
        ...
        const fetchChangelog = async () => {
            try {
                ...
                const response = await fetch(
                    "https://raw.githubusercontent.com/LumineDroid-Devices/official_devices/refs/heads/bellflower/changelogs/source.txt"
                );
                ...
                setAllChangelogs(parsed);
                setFilteredChangelogs(parsed);
                setAvailableDates(getUniqueDates(parsed));
                setError(null);
            } catch (err) {
                ...
                <section className="mb-12 sm:mb-20 text-center">
                    ...
                    >
                        ...
                        <p className="text-lg sm:text-xl text-neutral-600 dark:text-neutral-400 max-w-3xl mx-auto">
                            ...
                            improvements to LumineDroid ROM
                        </p>
                    </motion.div>
                </section>
                        ...
                        >
                            ...
                            <div className="flex flex-wrap gap-2">
                                {(searchQuery || selectedDate) && (
                                    ...
                                    >
                                        Filters: {filteredChangelogs.length}{" "}
                                        result
                    ...
                    <section className="mb-16 sm:mb-24">
                        ...
                        >
                            ...
                            <p className="text-neutral-700 dark:text-neutral-300 leading-relaxed mb-4">
                                LumineDroid continuously improves with regular
                                updates. Our changelog tracks all changes, bug
                                fixes, and new features added to the ROM.
                            </p>
